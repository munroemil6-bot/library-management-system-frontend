  ...
  if (loading) {
    return (
      <div className="container py-5 text-center">
        ...
        <p className="mt-3 text-muted">Loading your borrow history...</p>
      </div>
    );
  }
    ...
    <div className="container py-4 py-lg-5">
      <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-start gap-3 mb-4">
        <div>
          ...
          <p className="text-muted mb-0">
            {isAdmin
              ? "See which books are currently checked out and whether they have been returned."
              : "Track your current and past borrows in one place."}
          </p>
        </div>
        <div className="small text-muted">{records.length} records</div>
      </div>
      ...
      {records.length === 0 ? (
        <div className="rounded-4 p-5 text-center" style={{ background: "#f8fafc", border: "1px dashed #cbd5e1" }}>
          <p className="text-muted mb-0">No borrow records have been created yet.</p>
        </div>
      ) : (
          ...
          <div className="table-responsive">
            <table className="table align-middle mb-0">
              <thead style={{ background: "#f8fafc" }}>
                <tr>
                  <th className="px-4 py-3 fw-semibold small" style={{ color: "#64748b" }}>{isAdmin ? "Borrower" : "Book"}</th>
